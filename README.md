...
3. Connecting different Topologies
...
4. OSI model
Open System Interconnection model. Is a conceptual model
...
7. Syncronized data encoding
a. Unipolar: 1/+,  0/-
b. Polar
NRZ (Non-Return-Zero): 1/+,  0/-(or zero)

NRZS (NRZ-Space): 1/no change,  0/change
NRZI (NRZ-Inverted): 1/transition,  0/no transition
c. Bipolar: +, -,  zero
8. Self synchronizing data Encoding
a. Manchester: Data = Clock XOR Manchester
...
9. Radio Frecuency data Encoding: varying one or more properties of the carrier signal
...
13. Channel access control mechanism
...
TDMA (Time …)
b. Contention-based: any computer in the network can transmit data at any time (first come-first served)
c. Token-based or Reservation-based

14. CSMA/CD (Carrier Sense Multiple Access with Collision Detection)
...
17. Network Connection model
a. Connection-oriented comunication:
...
19. Network Message Fordwarding
...
d. Networks / Subnetworks: logically visible subdivision of an IP network
...
22. Host configuration in IP network

23. DCHP (Dynamic Host Configuration Protol)
...
24. ARP Protocol (Address Resolution Protocol)
...
26. Routing as routing and forwarding functions
Routing is the process of selecting best paths in a network.
In packet switching networks, routing directs(controls) packet forwarding through intermediate nodes (routers, bridges, gateways, firewalls, or switches. General-purposecomputers can also forward packets and perform routing, though they are not specialized hardware and may suffer from limited performance). 
...
27. Routing algorithm properties
a. Correctness: The routing should be done properly and correctly so that the packets may reach their proper destination.
b. Simplicity: The routing should be done in a simple manner so that the overhead is as low as possible. 
...
d. Stability: The routing algorithms should be stable under all possible circumstances.
e. Fairness: Every node connected to the network should get a fair chance of transmitting their packets. This is generally done on a first come first serve basis.
f. Optimality: The routing should be optimal in terms of throughput and minimizing mean packet delays. 

28. Adaptive routing algorithms (Dynamic routing algorithms)
...
29. Nonadaptive routing algorithms (Static routing algorithms)
...
31. Static routing algorithms
a. Shortest path routing and Dijkstra algorithm:
...
b. Routing by flooding

32. Dynamic routing algortithm
a. Distance Vector Routing and Bellman-Ford alg.
...
34. Main transport layer functions
a. Multiplexing/process-to-process communication:
...
b. End-to-end reliable communication:
...
a. Reliable: guarantees that all bytes received will be identical with bytes sent and in the correct order. Acknowledgment with retransmission technique is used: requires the receiver to respond with an acknowledgment message as it receives the data.
b. Ordered and error-checked: due to network congestion and other unpredictable network behavior, IP packets can be lost, duplicated, or delivered out of order. TCP detects these problems, requests retransmission of lost data, rearranges out-of-order data
...
38. TCP operation (3 phases)
a. TCP Connection stablishment
TCP 3-way handshake
...
a. Reliable
b. Ordered
c. Heavyweight: needs 3 packets to set up a socket connection. Also handles reliability and congestion control.
d. Streaming: data is read as a byte stream
UDP:
a. UNReliable
b. NOT Ordered
c. Lightweight: no ordering of messages, no tracking connections, etc
d. Datagrams: packets are sent individually and are checked for integrity only if they arrive
